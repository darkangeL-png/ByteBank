export function ValidarDebito(target:any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = function (this: any, valorDoDebito: number) {
        ...
        if (valorDoDebito <= 0) throw new Error("O valor a ser debitado precisa ser maior do que zero!");
         
        if (valorDoDebito > this.saldo) throw new Error("Seu saldo é insuficiente para realizar a operação!");
...
export function ValidarDeposito(target:any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = function (valorDoDeposito: number) {
        if (valorDoDeposito <= 0) throw new Error("O valor a ser depositado deve ser maior do que zero!");
